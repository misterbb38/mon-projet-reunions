...
function MeetingForm({ onAddMeeting }) {
  ...
  return (
    <form
      ...
      className="space-y-6 bg-white p-4 rounded shadow-sm"
    >
      <h2 className="text-xl font-semibold text-center">Créer une Réunion</h2>
      ...
      {/* ---- Ordre du jour (Items + Sous-items) ---- */}
      <div>
        <label className="block mb-2 font-medium">Ordre du jour</label>
        ...
        <div className="flex flex-col gap-2 sm:flex-row mb-4">
          ...
          >
            <option value="">Choisir un item parent</option>
            {agenda.map((item, idx) => (
              <option key={idx} value={idx}>
                {item.text}
              </option>
      ...
      <div>
        ...
        <textarea
          className="border p-2 w-full rounded"
          rows={3}
          placeholder="Décrivez brièvement les décisions..."
          value={summary}
          onChange={(e) => setSummary(e.target.value)}
        />
      </div>
